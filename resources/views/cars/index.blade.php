             ...
             <a href="{{ $car->status === 'active' ? route('cars.front.show', $car) : '#' }}" class="group block microsoft-container overflow-hidden">
                 ...
                 <div class="md:hidden flex items-center p-3 space-x-3">
                     ...
                     <div class="relative w-20 h-20 bg-white rounded-lg overflow-hidden flex-shrink-0 border">
                         @if($car->main_image)
                             <img src="{{ Storage::url($car->main_image) }}" alt="{{ $car->title }}" class="object-cover w-full h-full {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @elseif($car->images->first())
                             <img src="{{ Storage::url($car->images->first()->image_path) }}" alt="{{ $car->title }}" class="object-cover w-full h-full {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @else
                 ...
                 <div class="hidden md:block">
                     <div class="relative w-full aspect-square bg-white overflow-hidden border-b">
                         @if($car->main_image)
                             <img src="{{ Storage::url($car->main_image) }}" alt="{{ $car->title }}" class="object-cover w-full h-full transition-transform duration-500 group-hover:scale-105 {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @elseif($car->images->first())
                             <img src="{{ Storage::url($car->images->first()->image_path) }}" alt="{{ $car->title }}" class="object-cover w-full h-full transition-transform duration-500 group-hover:scale-105 {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @else
