...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-bold text-gray-800">تعديل الإعلان: <span class="font-normal text-[#4285F4]">{{ $car->title }}</span></h1>
...
<form action="{{ route('admin.cars.update', $car) }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-2xl shadow-md space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        {{-- عنوان الإعلان --}}
        <div>
            <label for="title" class="block text-sm font-medium text-gray-700 mb-2">عنوان الإعلان <span class="text-[#EA4335]">*</span></label>
        ...
        {{-- السعر --}}
        <div>
            <label for="price" class="block text-sm font-medium text-gray-700 mb-2">السعر (ريال) <span class="text-[#EA4335]">*</span></label>
        ...
        {{-- الحالة --}}
        <div>
            <label for="status" class="block text-sm font-medium text-gray-700 mb-2">الحالة <span class="text-[#EA4335]">*</span></label>
    ...
    {{-- قسم التصنيفات --}}
    <div>
        <h2 class="text-2xl font-bold mb-6 text-gray-800">التصنيفات</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($categories as $category)
                <div class="bg-gray-50 p-5 rounded-xl border border-gray-100">
                    <label for="category_{{ $category->id }}" class="block text-sm font-medium text-gray-700 mb-3">{{ $category->name }}</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="bg-blue-50 p-6 rounded-2xl border border-blue-100">
            <h3 class="text-xl font-bold mb-4 text-gray-800">الصورة الأساسية</h3>
            <div class="mb-5">
                <label class="block text-sm font-medium text-gray-700 mb-3">الصورة الحالية</label>
                @if($car->main_image)
                                         <img src="{{ asset('uploads/' . $car->main_image) }}"
 alt="الصورة الأساسية" class="w-36 h-36 object-cover rounded-xl shadow-md border border-gray-200">
                @else
        ...
        <div class="bg-green-50 p-6 rounded-2xl border border-green-100">
            <h3 class="text-xl font-bold mb-4 text-gray-800">الصور الإضافية</h3>
            <div class="mb-5">
                <label class="block text-sm font-medium text-gray-700 mb-3">الصور الحالية</label>
                @if($car->images->isNotEmpty())
                    <div class="flex flex-wrap gap-4">
                        @foreach($car->images as $image)
                            <div class="relative group">
                                                <img src="{{ asset('uploads/' . $car->main_image) }}"
 alt="صورة إضافية" class="w-28 h-28 object-cover rounded-xl shadow-md border border-gray-200">
