 ...
 <style>
     ...
     .custom-select {
        ...
        /* بما أن الصفحة RTL، السهم سيكون على اليسار. نزيد الحشوة اليمنى لتجنب تداخل النص مع السهم */
        padding-left: 2.5rem; 
     }

     /* تنسيق شريط "غير متوفرة" */
     .unavailable-ribbon {
         position: absolute;
     ...
     /* تعديل حجم الخط للشريط في الشاشات الكبيرة */
             ...
             <a href="{{ $car->status === 'active' ? route('cars.front.show', $car) : '#' }}" class="group block microsoft-container overflow-hidden">
                 ...
                 <div class="md:hidden flex items-center p-3 space-x-3">
                     ...
                     <div class="relative w-20 h-20 bg-white rounded-lg overflow-hidden flex-shrink-0 border">
                         @if($car->main_image)
                             <img   src="{{ Storage::disk('uploads')->url($car->main_image) }}" alt="{{ $car->title }}" class="object-cover w-full h-full {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @elseif($car->images->first())
                             <img    src="{{ Storage::disk('uploads')->url($image->image_path) }}"  alt="{{ $car->title }}" class="object-cover w-full h-full {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @else
                 ...
                 <div class="hidden md:block">
                     <div class="relative w-full aspect-square bg-white overflow-hidden border-b">
                         @if($car->main_image)
                             <img  src="{{ Storage::disk('uploads')->url($car->main_image) }}" alt="{{ $car->title }}" class="object-cover w-full h-full transition-transform duration-500 group-hover:scale-105 {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @elseif($car->images->first())
                             <img    src="{{ Storage::disk('uploads')->url($image->image_path) }}"  alt="{{ $car->title }}" class="object-cover w-full h-full transition-transform duration-500 group-hover:scale-105 {{ $car->status !== 'active' ? 'filter grayscale' : '' }}"/>
                         @else
